...
module.exports = {
    ...
    usage: "<all | country <name> | state <name> >",
    aliases: ["corona", "covid-19", "coronastats", "cstat"],
    run: async(client, message, args) => {
        const all = await covid.getAll();
        const country = await covid.getCountry({country: args.slice(1).join(" ")}).catch(message.channel.send("Data was not found!"));
        const state = await covid.getState({state: args.slice(1).join(" ")}).catch(e => message.channel.send("Data was not found!"));
        ...
        .setColor("RANDOM")
        if(!args[0]) return message.channel.send(embed).catch(message.channel.send("Data was not found!"));
